"""Verify interaction features are generated correctly (mul only, no div)."""
import numpy as np
...
class TestInteractionDepth:
    def test_synergistic_pair_gets_mul_feature(self):
    ...
    def test_antagonistic_pair_gets_mul_feature(self):
        X = np.array([[1.0, 2.0], [3.0, 4.0]], dtype=np.float32)
        names = ["A", "B"]
    ...
    def test_mixed_pairs_correct_count(self):
        ...
        X_aug, aug_names = generate_interaction_features(X, names, pairs)
        # 3 original + 1 (syn mul) + 1 (antag mul) = 5
