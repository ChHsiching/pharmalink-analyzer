...
def test_parsimony_coefficients_derived_from_preset():
    """Standard preset should produce parsimony coefficients [base*0.25, base, base*4]."""
...
def test_parsimony_coefficients_quick_preset():
    """Quick preset should produce parsimony coefficients [base*0.25, base, base*4]."""
    from app.ml.symbolic_regressor import run_pareto_regression
    X = np.array([[1, 2], [3, 4], [5, 6]], dtype=np.float32)
    y = np.array([1.0, 2.0, 3.0], dtype=np.float32)
    mock_models = [
...
def test_init_depth_forwarded_to_regressor():
    ...
    with patch("gplearn.genetic.SymbolicRegressor", return_value=mock_model) as MockSR:
        run_symbolic_regression(X, y, ["a", "b"], preset="quick")
        call_kwargs = MockSR.call_args[1]
        assert call_kwargs["init_depth"] == (2, 10)
